    ...
    <div class="MoreSoBox">
      ...
      <div class="menu">
        <div class="NarIndusInterPro">
          ...
          <div class="ProList">
            <ul>
              <li v-for="(item,index) in IndustrialInternetProList" :key="index"  @click="GoDetal(item.id,3)">
    ...
    export default {
      ...
      data(){
        return{
          ...
          PageSize:10,          //一页显示多少条
          Num:1,
          IndustrialInternetProList:[],    //工业互联网产品列表i
          SearchW:''
        }
      },
      methods:{
        ...
        GetLineList(){
          this.$http.get(this.$conf.env.IndustrialInternetPro + `page=${this.Num}&category=1`).then(res=>{
        ...
        SearchList(){
          ...
          this.$http.get(this.$conf.env.IndustrialInternetPro + `category=1&search=${this.SearchW}`).then(res=>{
            this.PrListNum = res.data.count
            this.IndustrialInternetProList = res.data.results
          }).catch(err=>{
            console.log(err)
        ...
        GoDetal(id,cur){
         let NewOpen =  this.$router.resolve({
          ...
          window.open(NewOpen.href, '_blank');
        }
      },
      created() {
        this.GetLineList()
